...
export default function Shop() {
  ...
  function filterData(mc,sc,br,min=-1,max=-1){
    ...
    if(mc==="All" && sc==="All" && br==="All")
      ...
      data = ProductStateData.filter(x => x.brand===br && x.maincategory===sc)
    else
  ...
  function postSearch(e){
    ...
    setProducts(ProductStateData.filter((x)=>x.name.toLowerCase().includes(ch) || 
    x.maincategory.toLowerCase() === ch || x.subcategory.toLowerCase() === ch || 
    x.brand.toLowerCase() === ch || x.color.toLowerCase() === ch ||   
    ...
    <div className="container-fluid my-3">
      <div className="row">
        <div className="col-md-2">
          ...
          <div className="mb-3">
            <h5 className="bg-primary text-center p-1 text-light">Price Filter</h5>
            <form onSubmit={priceFilter}>
              <div className="mb-3">
                <label>Min Amount</label>
                <input type="number" name="min" onChange={(e)=>setMin(e.target.value)} placeholder='Min Amount' className='form-control border-primary border-2'/>
              </div>
              <div className="mb-3">
                <label>Max Amount</label>
                <input type="number" name="max" onChange={(e)=>setMax(e.target.value)} placeholder='Max Amount' className='form-control border-primary border-2'/>
              </div>
